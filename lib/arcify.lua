    ...
    if e then
        if e.name and e.min and e.max then
            ...
            if e.is_control then
                ...
                local zero_value = e.controlspec:unmap(0)
                local zero_param_led = util.linlin(0, 1, control_start_angle, control_end_angle, zero_value)
                local left_led = math.min(zero_param_led, param_led)
                local right_led = math.max(zero_param_led, param_led)
                self.a_:segment(num, left_led, right_led, intensity)
            else
